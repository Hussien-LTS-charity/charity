...
describe("httpEditHealthHistoryHandler", () => {
    it("should update the Health History and return a success response", async () => {
        ...
        if (updatedHealthHistory) {
            expect(updatedHealthHistory.disease.diseaseName).toBe(updatedMockRequestBody.diseaseName);
            expect(updatedHealthHistory.id).toBe(newHealthHistory.body.healthHistory.id);
        }
    ...
    it("should return a 404 status code if invalid family ID provided", async () => {
        const response = await request
            .put(
                `/api/health-history/658/${mockRequestFamilyMemberBody.id}/123`
            )
            .send(updatedMockRequestBody);
    ...
    it("should return a 404 status code if invalid member needs ID provided", async () => {
        ...
        expect(response.status).toBe(404);
        expect(response.body.message).toBe("family member health history not found");
    });
...
describe("httpDeleteHealthHistoryHandler", () => {
    it("should delete the Health History and return a success response", async () => {
        ...
        const newHealthHistory = await request
            .post(`/api/health-history/${mockRequestFamilyMemberBody.FamilyId}/${mockRequestFamilyMemberBody.id}`)
            .send(firstMockRequestHealthHistoryBody);
        const response = await request
            .delete(`/api/health-history/${mockRequestFamilyMemberBody.FamilyId}/${mockRequestFamilyMemberBody.id}/${newHealthHistory.body.healthHistory.id}`)

        expect(response.status).toBe(200);
        expect(response.body.message).toBe("health history deleted successfully");

        const deletedFamily = await HealthHistory.findByPk(`${newHealthHistory.body.healthHistory.id}`);
    ...
    it("should return a 404 status code when an invalid family ID, member ID or need ID are provided", async () => {
        ...
        const firstResponse = await request
            .delete(`/api/health-history/12345/${mockRequestFamilyMemberBody.id}/123456`)
        ...
        const secondResponse = await request
            .delete(`/api/health-history/${mockRequestFamilyMemberBody.FamilyId}/12345/2345`)
        ...
        const thirdResponse = await request
            .delete(`/api/health-history/${mockRequestFamilyMemberBody.FamilyId}/${mockRequestFamilyMemberBody.id}/12345`)
...
describe("httpGetAllMembersNeedsHandler", () => {
    it("should return all Member Needs when a valid family ID is provided", async () => {
        ...
        const response = await request
            .get(`/api/health-history/${mockRequestFamilyMemberBody.FamilyId}`);

        expect(response.status).toBe(200);
        expect(response.body.count).toBeDefined();
        expect(response.body.familyMembersHealthHistory).toBeDefined();
        console.log("================================", response.body.familyMembersHealthHistory[0].healthHistory);

        expect(Array.isArray(response.body.familyMembersHealthHistory[0].healthHistory)).toBe(true);
    });
