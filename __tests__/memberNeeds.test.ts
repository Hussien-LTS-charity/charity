...
import { httpAddMemberNeedsHandler, httpDeleteMemberNeedsHandler, httpEditMemberNeedsHandler, httpGetAllMembersNeedsHandler, httpGetSpecificMemberNeedsHandler } from "../src/controllers/memberNeeds.controller";
...
describe.skip("httpEditMemberNeedsHandler", () => {
    it("should update the family and return a success response", async () => {
        ...
        await request
            ...
            .send(mockRequestFamilyMemberBody);
        await request
            .post(`/api/member-needs/${mockRequestFamilyBody.id}/${mockRequestFamilyMemberBody.id}`)
            .send(firstMockRequestNeedBody);


        const response = await request
            .put(`/api/member-needs/${mockRequestFamilyBody.id}/${mockRequestFamilyMemberBody.id}/${firstMockRequestNeedBody.id}`)
            .send(updatedMockRequestBody);

        expect(response.status).toBe(200);
        expect(response.body.message).toBe("Family member Needs updated successfully");
        ...
        const updatedFamilyMember = await request
            .get(`/api/family-member/${mockRequestFamilyBody.id}/${mockRequestFamilyMemberBody.id}`)

        if (updatedFamilyMember) {
            expect(updatedFamilyMember.body.familyMember).toBeDefined();
            expect(updatedFamilyMember.body.familyMember.id).toBe(mockRequestFamilyMemberBody.id);
            expect(updatedFamilyMember.body.familyMember.FamilyId).toBe(mockRequestFamilyBody.id);
        }
    });

    it("should return a 404 status code if invalid family ID provided", async () => {
        const response = await request
            .put(`/api/member-needs/658/${mockRequestFamilyMemberBody.id}/${firstMockRequestNeedBody.id}`)
            .send(updatedMockRequestBody);
    ...
    it("should return a 404 status code if invalid family member ID provided", async () => {
        const response = await request
            .put(`/api/member-needs/${mockRequestFamilyMemberBody.FamilyId}/11112/${firstMockRequestNeedBody.id}`)
            .send(updatedMockRequestBody);
    ...
    it("should return a 404 status code if invalid member needs ID provided", async () => {
        const response = await request
            .put(`/api/member-needs/${mockRequestFamilyMemberBody.FamilyId}/${mockRequestFamilyMemberBody.id}/11112`)
            .send(updatedMockRequestBody);
    ...
    it("should return a 500 status code and an error message when an error occurs in the handler", async () => {
        ...
        jest.spyOn(MemberNeeds, "update").mockRejectedValue(new Error("Test error"));
...
describe.skip("httpDeleteMemberNeedsHandler", () => {
    it("should delete the Member Needs and return a success response", async () => {
        ...
        await request
            ...
            .send(firstMockRequestNeedBody);
        const response = await request
            .delete(`/api/member-needs/${mockRequestFamilyMemberBody.FamilyId}/${mockRequestFamilyMemberBody.id}/${firstMockRequestNeedBody.id}`)

        expect(response.status).toBe(200);
        expect(response.body.message).toBe("Family member Needs deleted successfully");

        const deletedFamily = await MemberNeeds.findByPk(`${firstMockRequestNeedBody.id}`);
    ...
    it("should return a 404 status code when an invalid family ID, member ID or need ID are provided", async () => {
        ...
        const firstResponse = await request
            .delete(`/api/member-needs/12345/${mockRequestFamilyMemberBody.id}/${firstMockRequestNeedBody.id}`)
        ...
        const secondResponse = await request
            .delete(`/api/member-needs/${mockRequestFamilyMemberBody.FamilyId}/12345/${firstMockRequestNeedBody.id}`)
        ...
        const thirdResponse = await request
            .delete(`/api/member-needs/${mockRequestFamilyMemberBody.FamilyId}/${mockRequestFamilyMemberBody.id}/12345`)
